...
- **Problem**: `CPUChartWidget` and `MemoryChartWidget` are rendering their own legacy bordered containers *inside* the new premium glass cards.
...
- Simplify the animation logic. Apply the animation to the *content container* in `ProShell` or a single wrapper in `DashboardPage` rather than per-section.
...
- Standardize the `h-[300px]` constraint to the *container*, ensuring the inner chart fills it correctly without creating its own overflow boundaries.
