...
function Sparkline({ data, color = 'var(--primary)' }: { data: number[]; color?: string }) {
    ...
    return (
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-8 opacity-50 overflow-visible translate-y-1">
            <polyline
                fill="none"
                stroke={color}
                strokeWidth="1.5"
...
function DashboardContent() {
    ...
    const dataLatency = latest ? Math.max(0, new Date().getTime() - new Date(latest.serverTimestamp).getTime()) : 0;
        ...
        <div className="space-y-4 sm:space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
                ...
                <StatCard
                    ...
                    historyData={history.map(m => Math.max(0, (new Date(m.timestamp).getTime() % 1000) / 10))}
                    color="var(--info)"
                />
                <StatCard
                    label="Ping"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
                <Card className="lg:col-span-2 shadow-sm border-border/40 overflow-hidden">
                    <CardHeader className="flex-row items-center justify-between pb-2 bg-muted/5">
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5">Real-time load across all cores</p>
                        </div>
                ...
                <Card className="shadow-sm border-border/40 overflow-hidden">
                    <CardHeader className="pb-2 bg-muted/5">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">Active physical memory allocation</p>
                    </CardHeader>
                    <CardContent className="pt-6">
                        <div className="h-[200px] sm:h-[280px]">
...
function StatCard({ label, value, subLabel, icon, status, historyData = [], color }: {
    label: string;
    value: string;
    subLabel?: string;
        ...
        <Card className="relative overflow-hidden group hover:border-border transition-colors duration-300">
            <CardContent className="p-3 sm:p-4 relative z-10">
                <div className="flex items-center justify-between mb-1">
                    <span className="text-[10px] sm:text-xs font-medium uppercase tracking-wider text-muted-foreground/80">{label}</span>
