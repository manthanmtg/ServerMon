...
const { mockConnectDB, mockGetSession, mockFindById, mockCreate, mockFindByIdAndUpdate } = vi.hoisted(() => ({
...
describe('PUT /api/terminal/settings', () => {
    ...
    it('returns 403 when not admin', async () => {
        ...
        const res = await PUT(new Request('http://localhost', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: '{}' }));
    ...
    it('updates fontSize within bounds', async () => {
        ...
        const res = await PUT(new Request('http://localhost', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fontSize: 16 }) }));
        expect(res.status).toBe(200);
        const json = await res.json();
    ...
    it('clamps fontSize to min 10', async () => {
        ...
        await PUT(new Request('http://localhost', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fontSize: 5 }) }));
    ...
    it('clamps maxSessions to max 20', async () => {
        ...
        await PUT(new Request('http://localhost', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ maxSessions: 100 }) }));
        expect(mockFindByIdAndUpdate).toHaveBeenCalledWith(
            expect.anything(),
