...
const { mockConnectDB, mockSessionFind, mockSessionCreate, mockSessionCountDocuments, mockSessionFindOneAndUpdate, mockSessionDeleteOne, mockSessionDeleteMany, mockHistoryFindOne, mockGetSession, mockTerminalSettingsFindById } = vi.hoisted(() => ({
...
describe('GET /api/terminal/sessions', () => {
    ...
    it('returns 500 on DB error', async () => {
        mockSessionFind.mockReturnValue({ sort: () => ({ lean: () => Promise.reject(new Error('db error')) }) });
        const res = await GET();
...
describe('POST /api/terminal/sessions', () => {
    ...
    it('creates a new session', async () => {
        ...
        mockTerminalSettingsFindById.mockReturnValue({ lean: () => Promise.resolve({ maxSessions: 8 }) });
        ...
        const res = await POST(new Request('http://localhost', { method: 'POST', body: JSON.stringify({ label: 'Terminal 1' }) }));
...
describe('PUT /api/terminal/sessions', () => {
    ...
    it('returns 400 when sessionId missing', async () => {
        const res = await PUT(new Request('http://localhost', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) }));
        expect(res.status).toBe(400);
        const json = await res.json();
    ...
    it('updates session label', async () => {
        ...
        const res = await PUT(new Request('http://localhost', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'sess-1', label: 'New Label' }) }));
        expect(res.status).toBe(200);
        const json = await res.json();
    ...
    it('returns 404 when session not found', async () => {
        ...
        const res = await PUT(new Request('http://localhost', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'missing' }) }));
...
describe('DELETE /api/terminal/sessions', () => {
    ...
    it('deletes specific session', async () => {
        ...
        const res = await DELETE(new Request('http://localhost/api/terminal/sessions?sessionId=sess-1'));
        expect(res.status).toBe(200);
    });
