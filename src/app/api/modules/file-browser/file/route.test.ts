...
describe('GET /api/modules/file-browser/file (preview)', () => {
  ...
  it('returns preview for a file', async () => {
    ...
    const req = new NextRequest(
      'http://localhost/api/modules/file-browser/file?path=/home/user/test.txt'
    );
  ...
  it('returns editable file content when action=edit', async () => {
    ...
    const req = new NextRequest(
      'http://localhost/api/modules/file-browser/file?path=/home/user/test.txt&action=edit'
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
    const json = await res.json();
  ...
  it('returns download stream when action=download', async () => {
    ...
    const req = new NextRequest(
      'http://localhost/api/modules/file-browser/file?path=/home/user/test.txt&action=download'
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
  ...
  it('uses custom limits from settings', async () => {
    ...
    mockPreviewFile.mockResolvedValue(file);
    const req = new NextRequest(
      'http://localhost/api/modules/file-browser/file?path=/home/user/file.txt'
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
