...
describe('GET /api/modules/file-browser (list mode)', () => {
    beforeEach(() => { vi.clearAllMocks(); mockResolveBrowserPath.mockImplementation((p: string) => p); });
    ...
    it('returns tree when mode=tree', async () => {
        ...
        const req = new NextRequest('http://localhost/api/modules/file-browser?path=/home/user&mode=tree');
        const res = await GET(req);
        expect(res.status).toBe(200);
        const json = await res.json();
    ...
    it('clamps tree depth to 4', async () => {
        const tree = { name: 'user', kind: 'directory', children: [] };
        mockReadTree.mockResolvedValue(tree);
        const req = new NextRequest('http://localhost/api/modules/file-browser?path=/&mode=tree&depth=99');
        const res = await GET(req);
        expect(res.status).toBe(200);
...
describe('POST /api/modules/file-browser', () => {
    ...
    it('creates a file and returns 201', async () => {
        ...
        const req = new NextRequest('http://localhost/api/modules/file-browser', {
            ...
            body: JSON.stringify({ parentPath: '/home/user', name: 'new.txt', kind: 'file', content: 'hello' }),
        });
