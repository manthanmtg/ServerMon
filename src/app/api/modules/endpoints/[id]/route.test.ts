...
describe('PUT /api/modules/endpoints/[id]', () => {
    ...
    it('updates endpoint successfully', async () => {
        ...
        const res = await PUT(makeRequest({ name: 'Updated', slug: 'my-endpoint', method: 'GET', endpointType: 'script' }), makeContext('ep-1'));
        expect(res.status).toBe(200);
        const json = await res.json();
    ...
    it('returns 409 on slug conflict', async () => {
        ...
        const res = await PUT(makeRequest({ name: 'My Endpoint', slug: 'new-slug', method: 'GET', endpointType: 'script' }), makeContext('ep-1'));
    ...
    it('returns 500 on db error', async () => {
        ...
        const res = await PUT(makeRequest({ name: 'x', slug: 'x', method: 'GET', endpointType: 'script' }), makeContext('ep-1'));
        expect(res.status).toBe(500);
        const json = await res.json();
