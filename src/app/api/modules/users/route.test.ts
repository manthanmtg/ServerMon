...
const { mockGetSession, mockListOSUsers, mockListWebUsers, mockCreateOSUser, mockUpdateWebUserRole, mockToggleSudo, mockDeleteWebUser, mockDeleteOSUser } = vi.hoisted(() => ({
...
describe('POST /api/modules/users', () => {
    ...
    it('creates OS user', async () => {
        ...
        const res = await POST(makeRequest('POST', { type: 'os', username: 'newuser', shell: '/bin/bash' }));
        expect(res.status).toBe(200);
        const json = await res.json();
