...
describe('POST /api/modules/ai-agents/[sessionId]/kill', () => {
    ...
    it('kills session successfully', async () => {
        ...
        const res = await POST(new NextRequest('http://localhost', { method: 'POST' }), makeContext('session-abc'));
    ...
    it('returns 404 when session not found or could not be killed', async () => {
        ...
        const res = await POST(new NextRequest('http://localhost', { method: 'POST' }), makeContext('nonexistent'));
