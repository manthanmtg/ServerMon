...
const { mockConnectDB, mockCountDocuments, mockUserCreate, mockHashPassword, mockVerifyTOTPToken } = vi.hoisted(() => ({
...
describe('POST /api/setup/complete', () => {
    ...
    it('returns 403 when already set up', async () => {
        ...
        const res = await POST(makeRequest({ username: 'admin', password: 'pass', totpSecret: 'SEC', totpToken: '123456' }));
    ...
    it('returns 400 for invalid TOTP token', async () => {
        ...
        const res = await POST(makeRequest({ username: 'admin', password: 'pass', totpSecret: 'SEC', totpToken: '000000' }));
    ...
    it('creates admin with correct role', async () => {
        ...
        await POST(makeRequest({ username: 'admin', password: 'pass', totpSecret: 'S', totpToken: '1' }));
