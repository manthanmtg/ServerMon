...
function makeReq(): NextRequest {
  return new NextRequest('http://localhost/api/fleet/nodes/node-1/diagnose', {
    method: 'POST',
  });
}

describe('POST /api/fleet/nodes/[id]/diagnose', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('runs the client diagnostic chain and persists the run', async () => {
    mockGetSession.mockResolvedValue({ user: { username: 'admin', role: 'admin' } });
    mockFindById.mockResolvedValue({ _id: 'node-1' });

    const res = await POST(makeReq(), makeContext('node-1'));
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.kind).toBe('client');
    expect(json.targetId).toBe('node-1');
