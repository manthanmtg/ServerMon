...
const mockUser = {
    _id: { toString: () => 'user-1' },
    username: 'alice',
    passkeys: [] as { credentialID: string; publicKey: Buffer; counter: number; transports: string[]; createdAt: Date }[],
    save: mockSave,
...
describe('POST /api/auth/passkey/register/verify', () => {
    ...
    it('does not add duplicate credential', async () => {
        const user = {
            ...mockUser,
            passkeys: [{ credentialID: 'cred-new', publicKey: Buffer.from([1]), counter: 0, transports: [], createdAt: new Date() }],
        };
