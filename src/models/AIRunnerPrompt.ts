...
export interface IAIRunnerPromptAttachment {
  name: string;
  contentType: string;
  size: number;
  data: Buffer;
...
export interface IAIRunnerPrompt extends Document {
  ...
  tags: string[];
  attachments: IAIRunnerPromptAttachment[];
  createdAt: Date;
  updatedAt: Date;
}

const AIRunnerPromptAttachmentSchema = new Schema<IAIRunnerPromptAttachment>(
  {
...
const AIRunnerPromptSchema = new Schema<IAIRunnerPrompt>(
  {
    ...
    attachments: { type: [AIRunnerPromptAttachmentSchema], default: [] },
  },
  { timestamps: true }
);

AIRunnerPromptSchema.index({ updatedAt: -1 });
