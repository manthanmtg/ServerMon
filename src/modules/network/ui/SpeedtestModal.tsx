...
export default function SpeedtestModal({ isOpen, onClose }: SpeedtestModalProps) {
    ...
    const startTest = useCallback(() => {
        ...
        eventSource.onmessage = (event) => {
            ...
            if (data.type === 'ping') {
                setCurrentTest('ping');
                if (data.ping_ms) setResults(prev => ({ ...prev, ping: data.ping_ms!, jitter: data.jitter || null }));
                ...
                <div className="p-6 border-b border-border flex items-center justify-between bg-muted/30">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground font-medium">Testing server internet connectivity</p>
                        </div>
                    </div>
                    <button 
                        onClick={onClose}
                        ...
                        <div className="text-center space-y-8 py-10 w-full">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-muted-foreground max-w-xs mx-auto text-sm">
                                    This will measure the server&apos;s current download, upload speeds, and network latency.
                                </p>
                            </div>
                            <Button 
                                size="lg" 
                        ...
                        <div className="w-full space-y-8">
                            ...
                            <div className="flex flex-col items-center relative py-4">
                                ...
                                <div className="mt-4 text-center">
                                    ...
                                    <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest mt-1">Mbps</div>
                                </div>
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                ...
                                <MetricCard 
                                    ...
                                    completed={results.upload !== null && currentTest !== 'upload' || phase === 'completed'}
                                />
                            </div>
                            ...
                            <div className="bg-muted/40 rounded-2xl p-5 space-y-4 border border-border/50">
                                <div className="grid grid-cols-2 gap-y-4">
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-sm font-semibold truncate pr-4">{results.isp || (phase === 'testing' ? 'Identifying...' : '--')}</p>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
                                            <Milestone size={10} /> Server Location
                                        </p>
                                        <p className="text-sm font-semibold truncate">{results.server ? `${results.server}, ${results.location}` : (phase === 'testing' ? 'Locating...' : '--')}</p>
                                    </div>
                                </div>
