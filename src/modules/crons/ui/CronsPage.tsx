...
function ScheduleBuilder({ minute, hour, dayOfMonth, month, dayOfWeek, onChange }: ScheduleBuilderProps) {
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Expression:</span>
            ...
            <div className="grid grid-cols-5 gap-3">
                {[
                    ...
                    { label: 'Day of Week', field: 'dayOfWeek', value: dayOfWeek, placeholder: '0-7, 1-5, *' },
...
function describeScheduleClient(minute: string, hour: string, dom: string, month: string, dow: string): string {
    if (minute === '*' && hour === '*') return 'Runs every minute';
    if (minute === '0' && hour === '*') return 'Runs at the start of every hour';
    if (minute === '0' && hour === '0' && dom === '*' && month === '*' && dow === '*') return 'Runs daily at midnight';
    if (minute === '0' && hour === '0' && dom === '1' && month === '*' && dow === '*') return 'Runs monthly on the 1st at midnight';
    if (minute === '0' && hour === '0' && dom === '*' && month === '*' && dow === '0') return 'Runs weekly on Sunday at midnight';
...
function CronFormModal({ mode, initial, onClose, onSubmit }: CronFormModalProps) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        setSubmitting(true);
        try {
            await onSubmit({ minute, hour, dayOfMonth, month, dayOfWeek, command, comment: comment || undefined });
        } finally {
            setSubmitting(false);
        }
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-border bg-card shadow-xl mx-4">
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-5">
                    ...
                    <label className="block space-y-1">
                        <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Comment (optional)</span>
                        <input
                            type="text"
                            value={comment}
...
function NextRunsPanel({ job }: { job: CronJob }) {
    if (!job.enabled || job.nextRuns.length === 0) {
        return <p className="text-xs text-muted-foreground py-2">No upcoming runs (job is disabled or has no schedule).</p>;
    }

    return (
        <div className="space-y-1">
            <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-2">Next 5 runs</p>
...
export default function CronsPage() {
    ...
    const filteredJobs = useMemo(() => {
        ...
        if (filterSource !== 'all') {
            list = list.filter(j => filterSource === 'user' ? j.source === 'user' : j.source !== 'user');
        }
        ...
        const sorted = [...list].sort((a, b) => {
            ...
            switch (sortField) {
                ...
                case 'expression': cmp = a.expression.localeCompare(b.expression); break;
    ...
    async function toggleJob(job: CronJob) {
        ...
        try {
            ...
            toast({ title: job.enabled ? 'Job disabled' : 'Job enabled', description: data.message, variant: 'success' });
            await loadSnapshot();
        } catch (error: unknown) {
            toast({ title: 'Toggle failed', description: error instanceof Error ? error.message : 'Unknown error', variant: 'destructive' });
        } finally {
    ...
    async function handleEditSubmit(jobId: string, data: { minute: string; hour: string; dayOfMonth: string; month: string; dayOfWeek: string; command: string; comment?: string }) {
    ...
    return (
        <div className="space-y-6" data-testid="crons-page">
            ...
            <section className="rounded-[28px] border border-border/60 bg-[radial-gradient(circle_at_top_left,var(--primary)/0.18,transparent_40%),linear-gradient(180deg,var(--card),color-mix(in_oklab,var(--card)_92%,transparent))] p-4 shadow-sm sm:p-6">
                <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                    <div className="space-y-2">
                        ...
                        <div>
                            <h2 className="text-2xl font-semibold tracking-tight text-foreground">Cron Jobs Manager</h2>
                            <p className="text-sm text-muted-foreground">
                                Schedule, monitor, and manage cron jobs with visual schedule building and execution tracking.
                            </p>
                        </div>
                    </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                {[
                    { label: 'Total Jobs', value: snapshot?.summary.total ?? 0, icon: Clock, color: 'text-primary' },
                    { label: 'Active', value: snapshot?.summary.active ?? 0, icon: Play, color: 'text-success' },
                    { label: 'Disabled', value: snapshot?.summary.disabled ?? 0, icon: Pause, color: 'text-warning' },
                    { label: 'User Crons', value: snapshot?.summary.userCrons ?? 0, icon: Calendar, color: 'text-primary' },
                    { label: 'System Crons', value: snapshot?.summary.systemCrons ?? 0, icon: Timer, color: 'text-muted-foreground' },
            ...
            {snapshot?.summary.nextRunTime && (
                <Card className="border-border/60 bg-card/80">
                    <CardContent className="flex items-center gap-4 p-4">
                        ...
                        <div>
                            <p className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Next Scheduled Run</p>
                        ...
                        <table className="min-w-full text-sm">
                            <thead className="text-left text-xs uppercase tracking-[0.18em] text-muted-foreground bg-muted/20">
                                <tr>
                                    ...
                                    <th className="py-3 px-4 cursor-pointer select-none" onClick={() => toggleSort('expression')}>
                                        <span className="inline-flex items-center gap-1">Schedule <SortIcon field="expression" /></span>
                                    </th>
                                    <th className="py-3 px-4 cursor-pointer select-none" onClick={() => toggleSort('command')}>
                                        <span className="inline-flex items-center gap-1">Command <SortIcon field="command" /></span>
                                    </th>
                                    <th className="py-3 px-4 cursor-pointer select-none" onClick={() => toggleSort('user')}>
                                        <span className="inline-flex items-center gap-1">User <SortIcon field="user" /></span>
                                    </th>
                                    <th className="py-3 px-4 cursor-pointer select-none" onClick={() => toggleSort('source')}>
                                        <span className="inline-flex items-center gap-1">Source <SortIcon field="source" /></span>
                                    </th>
                                    <th className="py-3 px-4 cursor-pointer select-none" onClick={() => toggleSort('nextRun')}>
                                        <span className="inline-flex items-center gap-1">Next Run <SortIcon field="nextRun" /></span>
                                    </th>
                                                ...
                                                >
                                                    ...
                                                    <td className="py-3 px-4">
                                                        <div className="flex items-center gap-2">
                                                            <code className="text-xs font-mono bg-muted/50 px-1.5 py-0.5 rounded">{job.expression}</code>
                                                            <button
                                                                type="button"
                                                    ...
                                                    <td className="py-3 px-4">
                                                        ...
                                                        {job.comment && (
                                                            <p className="text-[10px] text-muted-foreground mt-0.5 truncate max-w-[280px]">{job.comment}</p>
                                                        )}
                                                    </td>
                                                    ...
                                                    <td className="py-3 px-4 text-xs">
                                                        {job.nextRuns[0]
                                                            ? <span className="text-foreground">{relativeTime(job.nextRuns[0])}</span>
                                                                ...
                                                                <div className="space-y-2">
                                                                    ...
                                                                    <div className="text-xs space-y-1">
                                                                        ...
                                                                        <div className="flex gap-2">
                                                                            ...
                                                                            <span className="text-foreground">{job.source}{job.sourceFile ? ` (${job.sourceFile})` : ''}</span>
                                                                        </div>
                                                                        {job.comment && (
                                                                            <div className="flex gap-2">
                                ...
                                {dir.scripts.length === 0 ? (
                                    ...
                                    <div className="space-y-1">
                                        {dir.scripts.map((script) => (
                                            <div key={script} className="flex items-center gap-2 text-xs py-1 px-2 rounded hover:bg-muted/30">
                ...
                <Card className="border-border/60">
                    ...
                    <CardContent>
                        {(snapshot?.recentLogs || []).length === 0 ? (
                            <p className="text-sm text-muted-foreground text-center py-8">No recent cron logs found.</p>
                        ) : (
