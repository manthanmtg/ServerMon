...
import { Clock, LoaderCircle, Play, Pause, Timer, Calendar } from 'lucide-react';
    ...
    <Card className="border-border/60">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <Badge
            variant={snapshot?.crontabAvailable ? 'success' : 'warning'}
            className="text-[10px]"
          >
      ...
      <CardContent>
        ...
        <div className="mt-3 flex items-center justify-between text-[10px] text-muted-foreground pt-3 border-t border-border/40">
          ...
          <span>
            {snapshot?.summary.userCrons ?? 0} user · {snapshot?.summary.systemCrons ?? 0} sys
          </span>
        </div>
      </CardContent>
    </Card>
