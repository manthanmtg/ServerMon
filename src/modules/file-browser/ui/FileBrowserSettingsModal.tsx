...
export default function FileBrowserSettingsModal({ settings, onClose, onSaved }: Props) {
  ...
  const canSave = useMemo(
    () => form.shortcuts.every((shortcut) => shortcut.label.trim() && shortcut.path.trim()),
    [form.shortcuts]
  );
  ...
  return (
    <Dialog
      ...
      description="Manage shortcuts and editor limits shown in the top bar."
      size="md"
      closeLabel="Close file browser settings"
      contentClassName="max-w-3xl animate-slide-up"
      footer={
        <>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.4fr,0.8fr]">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Absolute paths only. Root and home can be edited or removed.
              </p>
            </div>
          ...
          <div className="space-y-3 max-h-[26rem] overflow-y-auto pr-1">
            {form.shortcuts.map((shortcut, index) => (
              <div
                key={shortcut.id || `${index}-${shortcut.path}`}
        ...
        <div className="space-y-4">
          <Input
            ...
            onChange={(event) =>
              setForm((current) => ({ ...current, defaultPath: event.target.value }))
            }
            placeholder="/"
          />
          <Input
            ...
            onChange={(event) =>
              setForm((current) => ({ ...current, editorMaxBytes: Number(event.target.value) }))
            }
          />
          <Input
            ...
            onChange={(event) =>
              setForm((current) => ({ ...current, previewMaxBytes: Number(event.target.value) }))
            }
          />
          <div className="rounded-xl border border-border bg-secondary/20 p-4">
            <p className="text-sm font-medium text-foreground">Behavior</p>
            <p className="text-xs text-muted-foreground mt-2 leading-5">
              Preview uses the preview limit for text and log files. Edit mode uses the editor limit
              and keeps binary files read-only.
            </p>
          </div>
        </div>
      </div>
