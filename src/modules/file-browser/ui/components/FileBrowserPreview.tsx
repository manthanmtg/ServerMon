                ...
                <div className="flex flex-col border-b border-border/40">
                    <div className="flex items-center justify-between px-6 py-6">
                        <div className="flex items-center gap-5 min-w-0">
                            <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center text-primary shadow-inner border border-primary/10 shrink-0">
                                {entry.kind === 'image' ? <FileImage className="w-7 h-7" /> : 
                                 entry.kind === 'log' ? <Logs className="w-7 h-7" /> : 
                                 entry.kind === 'code' ? <FileCode2 className="w-7 h-7" /> : <FileText className="w-7 h-7" />}
                            </div>
                            <div className="min-w-0">
                                <h3 className="font-bold text-foreground truncate text-xl tracking-tight leading-tight mb-2" title={entry.name}>
                                    {entry.name}
                                </h3>
                        ...
                        <div className="flex items-center gap-2 ml-4">
                            {preview?.canWrite && !isEditing && (
                                <Button variant="outline" size="sm" onClick={onEdit} className="h-10 gap-2 shadow-sm border-border/60 hover:bg-accent hover:border-border font-semibold">
                                    <Pencil className="w-3.5 h-3.5" />
                                    Edit
                                </Button>
                            )}
                            <Button variant="outline" size="sm" onClick={onDownload} className="h-10 px-3.5 shadow-sm border-border/60 hover:bg-accent hover:border-border">
                                <Download className="w-4.5 h-4.5" />
                            </Button>
                            <div className="w-px h-6 bg-border/40 mx-2" />
                            <Button variant="ghost" size="icon" onClick={onClose} className="h-10 w-10 text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-all">
                    ...
                    {/* Metadata Section */}
                    <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-border/30 bg-secondary/5 border-t border-border/30 overflow-hidden">
                        <div className="px-6 py-4 flex flex-col gap-1.5 transition-colors hover:bg-secondary/10">
                            <span className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-[0.2em] leading-none">Modified</span>
                        ...
                        <div className="px-6 py-4 flex flex-col gap-1.5 transition-colors hover:bg-secondary/10">
                            <span className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-[0.2em] leading-none">Size</span>
                            <div className="flex items-center gap-2 text-xs font-bold text-foreground/90">
                        ...
                        <div className="px-6 py-4 flex flex-col gap-1.5 transition-colors hover:bg-secondary/10">
                            <span className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-[0.2em] leading-none">Permissions</span>
                            <div className="flex items-center gap-2 text-xs font-bold text-foreground/90">
                                <Shield className="w-3.5 h-3.5 text-primary/70" />
                                <code className="text-[11px] font-mono bg-background/80 px-1.5 py-0.5 rounded border border-border/40 text-primary shadow-sm tracking-tight">
                                    {entry.permissions}
                                </code>
                            </div>
                        </div>
