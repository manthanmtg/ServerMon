        ...
        <div className="flex flex-col h-full bg-black/90 text-zinc-100 rounded-xl border border-white/5 overflow-hidden shadow-2xl">
          ...
          <pre className="flex-1 overflow-auto p-6 font-mono text-xs leading-relaxed selection:bg-primary/30">
            {preview.tailLines.map((line, i) => (
              <div
                key={i}
                className="whitespace-pre-wrap py-0.5 border-l-2 border-transparent hover:border-primary/40 hover:bg-white/5 px-2 transition-colors"
              >
                {line}
              </div>
            ))}
    ...
    <div className="flex flex-col flex-1 min-h-0 bg-background/60">
      ...
      <div className="flex flex-col border-b border-border/40 shrink-0">
        ...
        <div className="flex items-center gap-3 px-4 pt-4 pb-2 md:px-5 md:pt-5 md:pb-2.5">
          ...
          <h3
            className="font-bold text-foreground truncate text-sm md:text-lg tracking-tight leading-tight flex-1 min-w-0"
            title={entry.name}
          >
            {entry.name}
          </h3>
          <Button
            ...
            size="icon"
            onClick={onClose}
            className="h-8 w-8 shrink-0 text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-all"
          >
            <X className="w-4 h-4" />
          </Button>
        </div>
        ...
        <div className="flex items-center gap-2 px-4 pb-3 md:px-5 md:pb-4 pl-[4.25rem] md:pl-[4.75rem]">
          <Badge
            variant="secondary"
            className="h-5 px-2 text-[10px] uppercase font-black tracking-widest bg-primary/20 text-primary border-primary/20 ring-1 ring-primary/10"
          >
            {entry.kind}
          </Badge>
          ...
          {preview?.canWrite && !isEditing && (
            <Button
              ...
              size="sm"
              onClick={onEdit}
              className="h-7 gap-1.5 px-2.5 text-[11px] shadow-sm border-border/60 hover:bg-accent hover:border-border"
            >
        ...
        <div className="flex flex-wrap md:grid md:grid-cols-3 md:divide-x divide-border/30 bg-secondary/5 border-t border-border/30 overflow-hidden min-w-0">
          <div className="px-4 py-2.5 md:px-6 md:py-4 flex items-center gap-2 md:flex-col md:items-start md:gap-1.5 transition-colors hover:bg-secondary/10">
            <span className="text-[9px] md:text-[10px] font-black text-muted-foreground/50 uppercase tracking-[0.15em] md:tracking-[0.2em] leading-none">
              Modified
            </span>
      ...
      <div className="flex-1 min-h-0 p-3 md:p-6">{renderContent()}</div>
    </div>
  );
}
