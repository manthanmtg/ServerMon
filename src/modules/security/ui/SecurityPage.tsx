                    ...
                    {categories.map(cat => (
                        <div key={cat}>
                            ...
                            <div className="space-y-1.5">
                                {snapshot.checks.filter(c => c.category === cat).map(check => (
                                    <div key={check.id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-accent/30 transition-colors">
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                            <Badge variant="secondary" className="text-[10px]">{check.severity}</Badge>
                                            <Badge
                                                variant={check.status === 'pass' ? 'success' : check.status === 'fail' ? 'destructive' : check.status === 'warn' ? 'warning' : 'secondary'}
                                                className="text-[10px]"
                                            >
                ...
                <Card className="border-border/60">
                    <CardHeader className="pb-3">
                        <div className="flex items-center justify-between">
                            ...
                            <Badge variant={snapshot.fail2ban.running ? 'success' : 'destructive'} className="text-[10px]">
                                    ...
                                    <div key={jail.name} className="p-3 rounded-lg border border-border/60 space-y-2">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <Badge variant={jail.currentlyBanned > 0 ? 'warning' : 'secondary'} className="text-[10px]">
                                                    {jail.currentlyBanned} banned
                                                </Badge>
                                                <span className="text-xs text-muted-foreground">{jail.totalBanned} total</span>
                                            </div>
                                        </div>
                            ...
                            <table className="w-full text-xs">
                                <thead>
                                    <tr className="border-b border-border bg-secondary/50">
                                        ...
                                        <th className="text-left font-medium text-muted-foreground px-4 py-2.5">IP Address</th>
                                        <th className="text-left font-medium text-muted-foreground px-4 py-2.5">Time</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {snapshot.recentLogins.map((login, i) => (
                                        <tr key={i} className="border-b border-border/50 hover:bg-accent/30 transition-colors">
                                            ...
                                            <td className="px-4 py-2.5 font-mono text-muted-foreground">{login.ip || '-'}</td>
                    ...
                    <CardContent className="p-0">
                        <div className="overflow-x-auto">
                            <table className="w-full text-xs">
                                <thead>
                                    <tr className="border-b border-border bg-secondary/50">
                                        <th className="text-left font-medium text-muted-foreground px-4 py-2.5">Package</th>
                                        <th className="text-left font-medium text-muted-foreground px-4 py-2.5">Current</th>
                                        <th className="text-left font-medium text-muted-foreground px-4 py-2.5">Available</th>
                                ...
                                <tbody>
                                    {snapshot.pendingUpdates.map((update, i) => (
                                        <tr key={i} className="border-b border-border/50 hover:bg-accent/30 transition-colors">
                                            ...
                                            <td className="px-4 py-2.5 font-mono text-muted-foreground">{update.currentVersion || '-'}</td>
                                            <td className="px-4 py-2.5 font-mono">{update.newVersion || '-'}</td>
                                            <td className="px-4 py-2.5">
                                                <Badge variant={update.isSecurity ? 'destructive' : 'secondary'} className="text-[10px]">
