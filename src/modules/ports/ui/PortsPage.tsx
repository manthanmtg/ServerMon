...
const WELL_KNOWN: Record<number, string> = {
    ...
    995: 'POP3S', 3000: 'Dev Server', 3306: 'MySQL', 5432: 'PostgreSQL',
    5672: 'RabbitMQ', 6379: 'Redis', 8080: 'HTTP Alt', 8443: 'HTTPS Alt',
                    ...
                    {checkResult && (
                        <div className={cn(
                            ...
                            {checkResult.available
                                ...
                                : <><XCircle className="w-4 h-4" /> Port {checkResult.port} is in use{checkResult.process ? ` by ${checkResult.process}` : ''}</>
                            }
                        </div>
                    )}
                </CardContent>
                        ...
                        <table className="w-full text-xs">
                            <thead>
                                <tr className="border-b border-border bg-secondary/50">
                                    ...
                                    <th className="text-left font-medium text-muted-foreground px-4 py-2.5">Protocol</th>
                                    <th className="text-left font-medium text-muted-foreground px-4 py-2.5">Address</th>
                                    <th className="text-left font-medium text-muted-foreground px-4 py-2.5">Process</th>
                            ...
                            <tbody>
                                {filtered.length === 0 ? (
                                    ...
                                    filtered
                                        ...
                                        .map((p, i) => (
                                            <tr key={`${p.protocol}-${p.port}-${p.address}-${i}`} className="border-b border-border/50 hover:bg-accent/30 transition-colors">
                                                <td className="px-4 py-2.5 font-mono font-semibold">{p.port}</td>
                                                <td className="px-4 py-2.5">
                                                    {WELL_KNOWN[p.port] ? (
                                                        <Badge variant="secondary" className="text-[10px]">{WELL_KNOWN[p.port]}</Badge>
                                                    ) : (
                                                ...
                                                <td className="px-4 py-2.5 font-mono text-muted-foreground">{p.address || '*'}</td>
                                                <td className="px-4 py-2.5 font-medium">{p.process || '-'}</td>
                                                <td className="px-4 py-2.5 font-mono text-muted-foreground">{p.pid ?? '-'}</td>
                                                <td className="px-4 py-2.5 text-muted-foreground">{p.user || '-'}</td>
                                                <td className="px-4 py-2.5">
                                                    <Badge variant="secondary" className="text-[10px]">{p.state}</Badge>
                                                </td>
                                            </tr>
                                        ))
                                )}
                ...
                <Card className="border-border/60">
                    <CardHeader className="pb-3">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                <Badge variant={snapshot.firewall.enabled ? 'success' : 'warning'} className="text-[10px]">
                        ...
                        {snapshot.firewall.rules.length === 0 ? (
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-xs">
                                    <thead>
                                        <tr className="border-b border-border bg-secondary/50">
                                            <th className="text-left font-medium text-muted-foreground px-4 py-2.5">Rule</th>
                                    ...
                                    <tbody>
                                        {snapshot.firewall.rules.map((rule, i) => (
                                            <tr key={i} className="border-b border-border/50 hover:bg-accent/30 transition-colors">
                                                ...
                                                <td className="px-4 py-2.5">
                                                    <Badge
                                                        variant={rule.action.toLowerCase().includes('allow') || rule.action.toLowerCase().includes('accept')
                                                            ? 'success' : 'destructive'}
                                                        className="text-[10px]"
                                                    >
                                                        {rule.action}
                                                ...
                                                <td className="px-4 py-2.5 text-muted-foreground">{rule.source || 'Any'}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
