...
export default function CertificatesPage() {
    ...
    const [renewResult, setRenewResult] = useState<{ domain: string; success: boolean; output: string } | null>(null);
    ...
    return (
        <div className="space-y-6">
                    ...
                    <CardContent className="pt-4 pb-4">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-4 text-xs">
                                <Badge variant={snapshot.certbotTimer.active ? 'success' : 'warning'} className="text-[10px]">
                                    {snapshot.certbotTimer.active ? 'Active' : 'Inactive'}
                                </Badge>
                                {snapshot.certbotTimer.nextRun && (
                                    <span className="text-muted-foreground">Next: <span className="font-medium text-foreground">{snapshot.certbotTimer.nextRun}</span></span>
                                )}
                                {snapshot.certbotTimer.lastRun && (
                                    <span className="text-muted-foreground">Last: <span className="font-medium text-foreground">{snapshot.certbotTimer.lastRun}</span></span>
                                )}
                            </div>
                        </div>
                    </CardContent>
            ...
            {/* Renewal Result */}
            {renewResult && (
                <Card className={cn('border', renewResult.success ? 'border-success/30 bg-success/5' : 'border-destructive/30 bg-destructive/5')}>
                    <CardContent className="pt-4 pb-4">
                        <div className="flex items-center gap-2 mb-2">
                            {renewResult.success ? <CheckCircle className="w-4 h-4 text-success" /> : <XCircle className="w-4 h-4 text-destructive" />}
                            <span className="text-sm font-medium">{renewResult.success ? 'Renewal successful' : 'Renewal failed'} for {renewResult.domain}</span>
                        </div>
                        <pre className="text-xs text-muted-foreground whitespace-pre-wrap max-h-32 overflow-y-auto">{renewResult.output}</pre>
                    </CardContent>
                </Card>
            )}
                ...
                <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <Badge variant={snapshot.certbotAvailable ? 'success' : 'warning'} className="text-[10px]">
                    ...
                    {snapshot.certificates.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {snapshot.certificates.map((cert) => (
                                <div key={cert.name} className={cn('rounded-lg border p-4 space-y-3', expiryBg(cert.daysUntilExpiry))}>
                                    <div className="flex items-center justify-between flex-wrap gap-2">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <Badge
                                                variant={cert.isExpired ? 'destructive' : cert.isExpiringSoon ? 'warning' : 'success'}
                                                className="text-[10px]"
                                            >
                                                {cert.isExpired ? 'Expired' : cert.isExpiringSoon ? 'Expiring Soon' : 'Valid'}
                                            </Badge>
                                            <span className={cn('text-xs font-semibold', expiryColor(cert.daysUntilExpiry))}>
