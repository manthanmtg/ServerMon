...
import { AlertTriangle, Eye, ShieldAlert, ShieldCheck, X } from 'lucide-react';
        ...
        <div className="flex items-start justify-between gap-4 border-b border-border/60 bg-background/85 px-6 py-5 backdrop-blur">
          <div className="max-w-3xl">
            <p className="text-[11px] uppercase tracking-[0.26em] text-primary/80">
              Workspace Collision View
            </p>
            <h3
              id="schedule-visualization-title"
              className="mt-2 text-2xl font-semibold tracking-tight"
            >
              Visualize schedule pressure before agents step on each other
            </h3>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              This map groups schedules by workspace and projects the next 24 hours so you can spot
              overlap windows, stacked runtime pressure, and places where one agent may still be
              active when another starts.
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="rounded-[24px] border border-warning/25 bg-warning/5 px-5 py-4">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-semibold">Why this view matters</p>
                  <p className="text-sm leading-6 text-muted-foreground">
                    When multiple enabled schedules target the same workspace, one run can still be
                    writing files, changing branches, or touching caches when the next one starts.
                    This doesn&apos;t block anything automatically, but it gives operators a clear
                    pre-flight view before they decide how much concurrency is acceptable.
                  </p>
                </div>
              </div>
            </div>

            {visualization.workspaces.length === 0 ? (
                          ...
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {entry.previewLimited ? (
                                <Badge variant="outline">Limited forecast</Badge>
                              ) : null}
                            </div>
                            ...
                            <div className="relative h-12 overflow-hidden rounded-xl border border-border/60 bg-muted/20">
                              ...
                              {entry.timelineWindows.length > 0 ? (
                                entry.timelineWindows.map((window, index) => {
                                  const left =
                                    ((window.startMs - visualization.horizonStartMs) / horizonMs) *
                                    100;
                                  const width = Math.max(
                                    ((window.endMs - window.startMs) / horizonMs) * 100,
                                    1.25
                                  );
