            ...
            <CardContent>
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className={cn('ml-auto font-semibold', (s?.error ?? 0) > 0 && 'text-destructive')}>{s?.error ?? 0}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                        ...
                        {sessions.slice(0, 3).map((session) => (
                            ...
                            >
                                ...
                                {session.environment.repository && (
                                    <span className="text-muted-foreground truncate">{session.environment.repository}</span>
                                )}
                                <Badge variant={statusVariant(session.status)} className="ml-auto text-[10px] shrink-0">
                                    {session.status}
                                </Badge>
                            </div>
                        ))}
