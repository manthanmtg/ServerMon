      ...
      label:
        form.endpointType === 'webhook'
          ...
          : form.endpointType === 'logic'
            ...
            : 'Code',
      shortLabel:
        form.endpointType === 'webhook' ? 'Hook' : form.endpointType === 'logic' ? 'Logic' : 'Code',
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <div className="flex items-center bg-background/40 rounded-xl border border-border/40 shadow-sm relative ml-1">
                ...
                {showCopyDropdown && (
                  <div
                    ref={dropdownRef}
                    className="absolute top-full right-0 mt-2 w-56 rounded-2xl border border-border/40 bg-card backdrop-blur-2xl shadow-2xl shadow-black/20 p-1.5 z-[70] animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-200 overflow-hidden ring-1 ring-white/[0.03]"
                  >
                    ...
                    {copyOptions.map((opt) => (
                      <button
                        ...
                        className="w-full text-left px-3 py-2 rounded-xl text-[11px] font-bold text-muted-foreground hover:text-primary hover:bg-primary/5 transition-all flex items-center justify-between group/opt active:scale-[0.98]"
                      >
                        {opt.label}
                        <Copy className="w-3 h-3 opacity-0 group-hover/opt:opacity-40 transition-opacity" />
                      </button>
                    ))}
                  </div>
                )}
