  ...
  if (logsLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 rounded-[3rem] border border-border/20 bg-muted/5 backdrop-blur-sm">
        ...
        <span className="text-xs font-bold text-muted-foreground/40 uppercase tracking-[0.2em]">Intercepting Logs...</span>
      </div>
    );
  }
  ...
  if (isCreating) {
    return (
      <div className="flex flex-col items-center py-20 text-center bg-muted/5 rounded-[3rem] border border-dashed border-border/40">
        ...
        <p className="text-sm font-black text-foreground uppercase tracking-tight mb-2">No active history</p>
        <p className="text-[11px] text-muted-foreground/60 max-w-[240px] leading-relaxed">
          The execution ledger will appear here once the endpoint has been instantiated and triggered.
        </p>
      </div>
    );
  }
  ...
  if (logs.length === 0) {
    return (
      <div className="flex flex-col items-center py-20 text-center bg-muted/5 rounded-[3rem] border border-dashed border-border/40">
        ...
        <p className="text-sm font-black text-foreground uppercase tracking-tight mb-2">Virgin Endpoint</p>
        <p className="text-[11px] text-muted-foreground/60 max-w-[240px] leading-relaxed">
          Awaiting the first interaction. All incoming calls will be logged with full payload transparency.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-10">
      <div className="flex items-center justify-between px-2 mb-4">
        <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-[0.2em]">Execution Ledger</label>
        <span className="text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full border border-primary/20">{logs.length} Entries</span>
      </div>
      {logs.map((entry) => (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <span className="text-[10px] text-muted-foreground font-mono font-bold">{entry.duration}ms latency</span>
          </div>
          ...
          {entry.error && (
            <div className="p-4 rounded-2xl bg-destructive/5 border border-destructive/20 animate-pulse">
               <div className="flex items-center gap-2 mb-1">
                 ...
                 <p className="text-[10px] font-bold text-destructive uppercase tracking-widest leading-none">Kernel Fault Exception</p>
               </div>
          ...
          <div className="flex items-center gap-6 px-2 pt-4 border-t border-border/20">
             {entry.requestMeta.ip && (
               <div className="flex flex-col gap-0.5">
                  <span className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-tighter">Source IP</span>
                  <span className="text-[10px] font-bold text-muted-foreground group-hover/log:text-foreground transition-colors">{entry.requestMeta.ip}</span>
               </div>
             )}
             {entry.requestMeta.userAgent && (
                <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                  <span className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-tighter">Client Agent</span>
                  <span className="text-[10px] font-bold text-muted-foreground truncate group-hover/log:text-foreground transition-colors">{entry.requestMeta.userAgent}</span>
                </div>
             )}
          </div>
        </div>
