    ...
    <div className="border-t border-border/40 bg-[#1e1e2e] animate-in slide-in-from-bottom-4 duration-500 shadow-[0_-20px_50px_rgba(0,0,0,0.3)] backdrop-blur-2xl relative z-50">
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
        <div className="flex items-center gap-3">
           ...
           <div>
             <span className="text-xs font-black text-white/90 uppercase tracking-widest">Interactive Test Console</span>
             <p className="text-[10px] text-white/40 font-medium">Draft and dispatch a test payload to your live endpoint</p>
           </div>
        </div>
        <div className="flex items-center gap-3">
           <button
            ...
            <div className="relative" ref={copyRequestMenuRef}>
              <button
                ...
                className={cn(
                  ...
                  showCopyRequestMenu && 'bg-white/10 border-primary/40 text-primary ring-2 ring-primary/20 shadow-lg'
                )}
              >
                <Copy className="w-3.5 h-3.5" />
                Export
                <ChevronDown className={cn("w-3 h-3 transition-transform duration-300", showCopyRequestMenu && "rotate-180")} />
              </button>
        ...
        <div className="p-6 overflow-y-auto relative custom-scrollbar bg-white/[0.02]">
          ...
          {testLoading ? (
            <div className="flex flex-col items-center justify-center h-[300px] rounded-2xl bg-[#0d0d0d] border border-dashed border-white/10 animate-pulse">
               ...
               <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em]">Interpreting Response...</span>
            </div>
          ) : testResult ? (
            ...
            <div className="flex flex-col items-center justify-center h-[300px] rounded-2xl bg-[#0d0d0d] border border-dashed border-white/5 group transition-all hover:border-white/10">
               ...
               <p className="text-xs font-black text-white/20 uppercase tracking-widest group-hover:text-white/40 transition-colors">Awaiting Dispatch</p>
               <p className="text-[10px] text-white/10 mt-2 font-medium">Press Fire Request or <kbd className="bg-white/5 px-1.5 py-0.5 rounded border border-white/10">⌘Enter</kbd> to launch</p>
            </div>
          )}
        </div>
      </div>
