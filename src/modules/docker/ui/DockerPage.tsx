...
function DataTable({
    ...
    onDelete: (id: string, type: DataTab) => void;
}) {
    return (
                ...
                <table className="min-w-full text-sm" data-testid="docker-images-table">
                    <thead className="text-left text-xs uppercase tracking-[0.18em] text-muted-foreground">
                        <tr>
                            ...
                            <th className="pb-3 pr-4">Using</th>
                    ...
                    <tbody>
                        {images.map((image) => (
                            <tr key={image.id} className="border-t border-border/60">
                                ...
                                <td className="py-3 pr-4">{image.containersUsing}</td>
                                <td className="py-3">
                                    <Button
                                        size="sm"
                                        variant="ghost"
                ...
                <table className="min-w-full text-sm" data-testid="docker-volumes-table">
                    <thead className="text-left text-xs uppercase tracking-[0.18em] text-muted-foreground">
                        <tr>
                            ...
                            <th className="pb-3 pr-4">Mountpoint</th>
                            <th className="pb-3">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {volumes.map((volume) => (
                            <tr key={volume.name} className="border-t border-border/60">
                                ...
                                <td className="py-3 pr-4 font-mono text-xs text-muted-foreground">{volume.mountpoint || 'n/a'}</td>
                                <td className="py-3">
                                    <Button
                                        size="sm"
                                        variant="ghost"
                    ...
                    <tbody>
                        {networks.map((network) => (
                            <tr key={network.id} className="border-t border-border/60">
                                ...
                                <td className="py-3 pr-4 font-mono text-xs text-muted-foreground">{network.id.slice(0, 12)}</td>
                                <td className="py-3">
                                    <Button
                                        size="sm"
                                        variant="ghost"
...
export default function DockerPage() {
    ...
    async function deleteAsset(id: string, type: DataTab) {
                                                ...
                                                <tr className="border-t border-border/60 align-top">
                                                    ...
                                                    <td className="py-3">
                                                                <div className="flex flex-wrap gap-2">
                                                                    ...
                                                                    <Button size="sm" variant="outline" className="text-destructive hover:bg-destructive/10 hover:text-destructive" disabled={pendingActionId === container.id} onClick={() => runAction(container.id, 'remove')}>
                                                                        <Trash2 className="h-4 w-4" />
                                                                    </Button>
                                ...
                                <div className="grid gap-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="rounded-xl border border-border/60 bg-muted/20 p-3">
                                            ...
                                            <p className="mt-1 font-semibold truncate">{snapshot?.daemon.storageDriver || 'Unavailable'}</p>
                                        </div>
                                        <div className="rounded-xl border border-border/60 bg-muted/20 p-3">
                                            <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Disk</p>
                                            <p className="mt-1 font-semibold">{formatBytes(snapshot?.diskUsage.totalBytes || 0)}</p>
                                            <p className="text-[10px] text-muted-foreground">{snapshot?.diskUsage.usedPercent.toFixed(1) || '0.0'}% used</p>
                                        </div>
                                    </div>
                                </div>
                            </CardContent>
                                        ...
                                        <div key={alert.id} className={cn(
                                            ...
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="mt-1 text-[9px] uppercase tracking-[0.18em] text-muted-foreground">{alert.source} · {relativeTime(alert.lastSeenAt)}</p>
                                                </div>
                                            </div>
                                        </div>
                                    )) : (
                                        ...
                                        <div key={event.id} className="rounded-xl border border-border/60 bg-muted/20 p-3">
                                            <div className="flex items-center justify-between gap-3">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        <p className="font-medium text-xs">{event.actor}</p>
                                                        <p className="text-[9px] uppercase tracking-[0.18em] text-muted-foreground">{event.type} · {event.action}</p>
                                                    </div>
                                                </div>
                                                <span className="text-[10px] text-muted-foreground">{relativeTime(event.time)}</span>
                                            </div>
                                        </div>
                                    )) : (
                                        <div className="rounded-xl border border-border/60 bg-muted/20 p-3 text-sm text-muted-foreground">
