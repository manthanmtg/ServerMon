  ...
  const getClosedByDisplay = (record: HistoryRecord) => {
    ...
    if (record.closedBy === 'timeout-autokill') {
      return (
        <Badge
          variant="outline"
          className="text-[10px] text-warning border-warning/20 bg-warning/5"
        >
          Timeout
        </Badge>
      );
    ...
    if (record.closedBy === 'server-restart') {
      return (
        <Badge
          variant="outline"
          className="text-[10px] text-muted-foreground border-border bg-secondary/30"
        >
          Restart
        </Badge>
      );
    ...
    if (record.closedBy === 'process-exit') {
      return record.exitCode === 0 ? (
        <Badge
          variant="outline"
          className="text-[10px] text-success border-success/20 bg-success/5"
        >
          Exit 0
        </Badge>
      ) : (
        <Badge
          variant="outline"
          className="text-[10px] text-destructive border-destructive/20 bg-destructive/5"
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Review metadata of historical terminal sessions
              </p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="h-64 flex flex-col items-center justify-center gap-2 p-6 text-center">
              ...
              <p className="text-xs text-muted-foreground">
                Historical sessions will appear here once they are closed
              </p>
            </div>
          ) : (
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-border/50">
                {history.map((record) => (
                  <tr key={record._id} className="text-xs transition-colors hover:bg-secondary/20">
                    ...
                    <td className="px-4 py-3.5">
                      {record.closedAt ? (
                        ...
                        <Badge variant="success" className="text-[9px] h-4 px-1 leading-none">
                          Active
                        </Badge>
                      )}
                    </td>
                    <td className="px-4 py-3.5 min-w-[120px]">{getClosedByDisplay(record)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
