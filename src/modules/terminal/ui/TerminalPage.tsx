...
export default function TerminalPage() {
    ...
    const resetAll = async () => {
        try {
            ...
            const data = await res.json();
            if (data.sessions) {
                const resetTabs = data.sessions.map((s: SessionTab) => ({ ...s, status: 'connecting' as const }));
                setTabs(resetTabs);
    ...
    const commitRename = async (sessionId: string) => {
        ...
        try {
            ...
            setTabs((prev) => prev.map((t) => t.sessionId === sessionId ? { ...t, label: trimmed } : t));
        } catch { /* ignore */ }
        setEditingTabId(null);
    };

    const updateTabStatus = useCallback((sessionId: string, status: 'connected' | 'disconnected' | 'connecting') => {
        ...
        <div className="h-full flex flex-col gap-2">
                        ...
                        >
                            <span
                                className={cn(
                                    ...
                                    tab.status === 'connecting' ? 'bg-warning animate-pulse' : 'bg-destructive',
                                )}
                            />
                            {editingTabId === tab.sessionId ? (
                                <form
                                    onSubmit={(e) => { e.preventDefault(); commitRename(tab.sessionId); }}
                                ...
                                >
                                    <input
                                        ...
                                        maxLength={20}
                                    />
                                    <button type="submit" className="p-0.5 hover:bg-white/10 rounded"><Check className="w-3 h-3" /></button>
                                </form>
                            ) : (
                                <span
            ...
            <div className="flex-1 rounded-xl border border-border bg-card overflow-hidden flex flex-col">
                ...
                <div className="flex-1 relative">
                    {tabs.map((tab) => (
                        <div
                            key={tab.sessionId}
                            className={cn('absolute inset-0', tab.sessionId === activeTabId ? 'z-10' : 'z-0 invisible')}
                        >
                            <TerminalUI
            ...
            {showResetConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowResetConfirm(false)}>
                    ...
                    <div className="relative rounded-xl border border-border bg-card p-6 max-w-sm w-full animate-slide-up" onClick={(e) => e.stopPropagation()}>
                        <h3 className="text-base font-semibold text-foreground mb-2">Reset all terminals?</h3>
                        <p className="text-sm text-muted-foreground mb-5">
                            This will close all terminal sessions and create a single new one. Active connections will be lost.
                        </p>
