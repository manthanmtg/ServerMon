...
import { 
    Activity, Zap, Database, Trash2,
    Brain, RefreshCcw, List, Terminal
} from 'lucide-react';
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    <div className={cn(
                        ...
                        <div className={cn(
                            ...
                            <Activity className={cn("w-6 h-6", isHighPressure ? "text-destructive" : "text-primary")} />
                        </div>
                    ...
                    <div className="flex-[2] grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                            { label: 'Total RAM', value: latest ? formatBytes(latest.memTotal) : '--', icon: Database },
                            { label: 'Used RAM', value: latest ? formatBytes(latest.memUsed) : '--', icon: Zap },
                            { label: 'Total Swap', value: detailedStats ? formatBytes(detailedStats.swaptotal) : '--', icon: Terminal },
                            { label: 'Used Swap', value: detailedStats ? formatBytes(detailedStats.swapused) : '--', icon: Brain },
                        ].map((stat, i) => (
                            <div key={i} className="p-4 rounded-2xl bg-card border border-border shadow-sm flex flex-col justify-between">
                    ...
                    <div className="p-6 rounded-3xl bg-card border border-border shadow-md">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground font-medium">Real-time telemetry stream</p>
                            </div>
                    ...
                    <div className="p-6 rounded-3xl bg-card border border-border shadow-md">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground font-medium">Internal allocation analysis</p>
                            </div>
                            <button 
                                onClick={fetchData} 
                        ...
                        <div className="space-y-4">
                            {[
                                { label: 'Active', value: detailedStats?.active, total: detailedStats?.total, color: 'bg-primary' },
                                { label: 'Cached', value: detailedStats?.cached, total: detailedStats?.total, color: 'bg-emerald-500' },
                                { label: 'Buffers', value: detailedStats?.buffers, total: detailedStats?.total, color: 'bg-amber-500' },
                                { label: 'Slab', value: detailedStats?.slab, total: detailedStats?.total, color: 'bg-indigo-500' },
                            ].map((item, i) => {
                                ...
                                <div className="p-4 rounded-2xl bg-accent/20 border border-border/30">
                                    ...
                                    <div className="h-3 rounded-full bg-background overflow-hidden p-0.5 border border-border/50">
                                        <div 
                                            ...
                                            style={{ width: `${(detailedStats.swapused / detailedStats.swaptotal) * 100}%` }}
                                        />
                                    </div>
                                </div>
                            ) : (
                    ...
                    <div className="flex items-center justify-between mb-8">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground font-medium">Sorted by RSS (Resident Set Size)</p>
                            </div>
                        </div>
                    </div>
