...
export default function NginxPage() {
    ...
    const [reloadResult, setReloadResult] = useState<{ success: boolean; output: string } | null>(null);
        ...
        <div className="space-y-6">
                ...
                <Card className="border-border/60">
                    <CardContent className="pt-4 pb-4">
                        <div className="flex items-center gap-3">
                            <div className={cn('w-10 h-10 rounded-lg flex items-center justify-center', snapshot.status.running ? 'bg-success/10' : 'bg-destructive/10')}>
                                {snapshot.status.running ? <Play className="w-5 h-5 text-success" /> : <XCircle className="w-5 h-5 text-destructive" />}
                            </div>
                            <div>
            ...
            <Card className="border-border/60">
                ...
                <CardContent>
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        >
                            {testing ? <LoaderCircle className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />}
                            Test Config
                        </button>
                        <button
                        ...
                        >
                            {reloading ? <LoaderCircle className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                            Reload Nginx
                        </button>
                        <button
                            onClick={load}
                    ...
                    {testResult && (
                        <div className={cn(
                            ...
                            testResult.success ? 'bg-success/5 border-success/20' : 'bg-destructive/5 border-destructive/20',
                        )}>
                            <div className="flex items-center gap-2 mb-1">
                                {testResult.success ? <CheckCircle className="w-4 h-4 text-success" /> : <XCircle className="w-4 h-4 text-destructive" />}
                                <span className="text-sm font-medium">{testResult.success ? 'Config test passed' : 'Config test failed'}</span>
                            </div>
                            <pre className="text-xs text-muted-foreground whitespace-pre-wrap max-h-24 overflow-y-auto">{testResult.output}</pre>
                        </div>
                    )}
                    ...
                    {reloadResult && (
                        <div className={cn(
                            'mt-3 rounded-lg border p-3',
                            reloadResult.success ? 'bg-success/5 border-success/20' : 'bg-destructive/5 border-destructive/20',
                        )}>
                            <div className="flex items-center gap-2">
                                {reloadResult.success ? <CheckCircle className="w-4 h-4 text-success" /> : <XCircle className="w-4 h-4 text-destructive" />}
                    ...
                    {snapshot.virtualHosts.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {snapshot.virtualHosts.map((vhost) => (
                                <div key={vhost.name} className="rounded-lg border border-border/60 overflow-hidden">
                                    <button
                                        onClick={() => setExpandedVhost(expandedVhost === vhost.name ? null : vhost.name)}
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Badge variant={vhost.enabled ? 'success' : 'secondary'} className="text-[10px]">
                                    ...
                                    {expandedVhost === vhost.name && (
                                        <div className="border-t border-border/60 p-4 bg-secondary/30 space-y-2 text-xs">
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                                <div><span className="text-muted-foreground">Listen:</span> <span className="font-medium">{vhost.listenPorts.join(', ') || 'N/A'}</span></div>
                                                <div><span className="text-muted-foreground">Root:</span> <span className="font-mono">{vhost.root || 'N/A'}</span></div>
                                                {vhost.proxyPass && (
                                                    <div><span className="text-muted-foreground">Proxy Pass:</span> <span className="font-mono">{vhost.proxyPass}</span></div>
                                                )}
                                                <div><span className="text-muted-foreground">File:</span> <span className="font-mono">{vhost.filename}</span></div>
                                            </div>
                                            {vhost.raw && (
                                                <details className="mt-2">
                                                    <summary className="cursor-pointer text-muted-foreground hover:text-foreground text-[11px]">View config</summary>
            ...
            {snapshot.status.configPath && (
                <div className="text-xs text-muted-foreground text-center">
                    Config: <code className="font-mono bg-secondary px-1.5 py-0.5 rounded">{snapshot.status.configPath}</code>
                </div>
            )}
        </div>
    );
