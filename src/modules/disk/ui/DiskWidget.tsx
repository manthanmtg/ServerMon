...
export default function DiskWidget() {
        ...
        return (
            <Card className="h-full border-border/50 bg-card/50 backdrop-blur-sm">
                <CardHeader className="py-3 px-4 flex flex-row items-center justify-between space-y-0">
                    <CardTitle className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Disk Usage</CardTitle>
    ...
    const primaryDisk = (latest.disks || []).find(d => d.mount === '/System/Volumes/Data') || 
                       (latest.disks || []).find(d => d.mount === '/') || 
                       latest.disks?.[0];
    const io = latest.io;

    const diskLabel = primaryDisk.mount === '/System/Volumes/Data' ? 'Main Disk' : 
                     primaryDisk.mount === '/' ? 'System Usage' : 'Disk Usage';
            ...
            <CardContent className="space-y-4">
                <div>
                    <div className="flex items-end justify-between mb-1.5">
                        ...
                        <span className="text-[11px] font-medium text-muted-foreground">{formatBytes(primaryDisk.used, unitSystem)} / {formatBytes(primaryDisk.size, unitSystem)}</span>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4 pt-1">
                    <div className="space-y-1">
                        <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                            <span className="w-3.5 h-3.5 rounded-sm bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-[8px] font-black text-emerald-500">R</span> Read
                        </span>
                    ...
                    <div className="space-y-1">
                        <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                            <span className="w-3.5 h-3.5 rounded-sm bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-[8px] font-black text-blue-500">W</span> Write
                        </span>
                        <p className="text-sm font-semibold truncate">
