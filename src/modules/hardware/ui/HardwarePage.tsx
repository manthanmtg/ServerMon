...
export default function HardwarePage() {
  ...
  const memPct =
    snapshot.memory.total > 0
      ? ((snapshot.memory.used / snapshot.memory.total) * 100).toFixed(1)
      : '0';
        ...
        <CardContent>
          {snapshot.cpuTemperature.main > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {snapshot.cpuTemperature.max > 0 &&
                snapshot.cpuTemperature.max !== snapshot.cpuTemperature.main && (
                ...
                <tbody>
                  {snapshot.memoryLayout.map((m, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5">
                        {m.clockSpeed > 0 ? `${m.clockSpeed} MHz` : '-'}
                      </td>
                ...
                <tbody>
                  {snapshot.disks.map((d, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5">
                        {d.temperature > 0 ? `${d.temperature}°C` : '-'}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <Card className="border-border/60">
              ...
              <CardContent className="space-y-1.5 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{snapshot.bios.vendor}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Version</span>
                  <span className="font-medium">{snapshot.bios.version}</span>
                </div>
                {snapshot.bios.releaseDate && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Date</span>
                    <span className="font-medium">{snapshot.bios.releaseDate}</span>
                  </div>
                )}
              </CardContent>
            </Card>
          ...
          {snapshot.baseboard.manufacturer && (
            <Card className="border-border/60">
              ...
              <CardContent className="space-y-1.5 text-xs">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Manufacturer</span>
                  <span className="font-medium">{snapshot.baseboard.manufacturer}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Model</span>
                  <span className="font-medium">{snapshot.baseboard.model}</span>
                </div>
                {snapshot.baseboard.memSlots > 0 && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Memory Slots</span>
                    <span className="font-medium">{snapshot.baseboard.memSlots}</span>
                  </div>
                )}
              </CardContent>
            </Card>
