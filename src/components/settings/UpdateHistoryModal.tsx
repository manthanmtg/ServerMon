                  ...
                  <div className="flex items-center gap-3">
                    <Badge
                      ...
                      className={cn(
                        ...
                        selectedRun.status === 'running' &&
                          'animate-pulse shadow-[0_0_15px_rgba(var(--warning),0.4)]'
                      )}
                    >
                      {selectedRun.status}
                    </Badge>
              ...
              <div className="flex-1 bg-[#050505]/60 relative min-h-0">
                {detailsLoading ? (
                  <div className="flex flex-col items-center justify-center h-full gap-5">
                    ...
                    <span className="text-xs font-black uppercase tracking-[0.4em] text-muted-foreground/30 animate-pulse">
                      Synchronizing Logs
                    </span>
                  </div>
                ) : (
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-40 px-10 text-center animate-in fade-in zoom-in-95 duration-700">
                  ...
                  <p className="text-sm text-muted-foreground mt-3 max-w-[320px] leading-[1.6]">
                    Your update logs are empty. New installation artifacts will appear here after
                    successfull update runs.
                  </p>
                </div>
              ) : (
                      ...
                      <div className="flex items-center gap-6 sm:gap-8 relative z-10">
                        <div
                          className={cn(
                            ...
                            run.status === 'completed'
                              ...
                              : run.status === 'running'
                                ? 'bg-warning/10 border-warning/20 shadow-warning/10'
                        ...
                        <div className="text-left">
                          <div className="flex items-center gap-4">
                            ...
                            <Badge
                              variant="outline"
                              className="text-[10px] font-mono font-bold h-5 px-3 bg-black/40 border-white/5 text-muted-foreground/60 rounded-lg"
                            >
                              #{run.runId.slice(-8)}
                            </Badge>
                          </div>
                            ...
                            {run.status === 'running' ? (
                              <div className="flex items-center gap-2 text-warning/80">
                                ...
                                <span className="font-black uppercase text-[10px] tracking-[0.1em]">
                                  ELAPSED:{' '}
                                  {formatDuration(
                                    Math.round((now - new Date(run.startedAt).getTime()) / 1000)
                                  )}
                                </span>
                              </div>
                            ) : (
