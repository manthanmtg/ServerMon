...
export class CodexAdapter implements AgentAdapter {
    ...
    private async buildSession(rolloutPath: string, codexDir: string, username: string): Promise<AgentSession | null> {
    ...
    private async extractSessionData(rolloutPath: string, codexDir: string): Promise<{
        ...
        try {
            ...
            try {
                ...
                if (fs.existsSync(indexPath)) {
                    const rolloutId = path.basename(rolloutPath).replace(/^rollout-.*-/, '').replace(/\.jsonl$/, '');
