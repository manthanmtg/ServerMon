...
class UsersService {
        ...
        try {
            ...
            for (const line of lines) {
                ...
                if (uidNum >= 1000 || username === 'root') {
                    ...
                    const hasSudo = groups.includes('sudo') || groups.includes('wheel') || username === 'root';
    ...
    public async listWebUsers(): Promise<WebUser[]> {
        try {
            ...
            return docs.map((d: { _id: unknown; username: string; role: 'admin' | 'user'; isActive: boolean; lastLoginAt?: Date; createdAt?: Date }) => ({
                id: String(d._id),
                username: d.username,
                role: d.role,
                isActive: d.isActive,
