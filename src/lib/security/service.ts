...
async function getSshConfig(): Promise<SshConfig | null> {
    try {
        ...
        const allowUsersMatch = content.match(/^\s*AllowUsers\s+(.+)/mi);
        const allowGroupsMatch = content.match(/^\s*AllowGroups\s+(.+)/mi);
    ...
    try {
        if (process.platform === 'linux') {
            ...
            for (const line of raw.split('\n')) {
                ...
                if (parts.length < 4 || parts[0] === '' || line.includes('wtmp begins') || line.includes('reboot')) continue;
                entries.push({
                    user: parts[0],
                    ip: parts[2] || '',
...
function runSecurityChecks(
    ...
    if (ssh) {
        checks.push({
            ...
            status: ssh.permitRootLogin === 'no' ? 'pass' : (ssh.permitRootLogin === 'prohibit-password' ? 'warn' : 'fail'),
    ...
    checks.push({
        ...
        details: securityUpdates.length === 0
...
function getMockData(): SecuritySnapshot {
    const checks: SecurityCheck[] = [
        { id: 'firewall-enabled', category: 'Firewall', title: 'Firewall is enabled', description: '', status: 'pass', details: 'UFW is active', severity: 'critical' },
        { id: 'ssh-root-login', category: 'SSH', title: 'Root login is disabled', description: '', status: 'pass', details: 'PermitRootLogin = no', severity: 'high' },
        { id: 'ssh-password-auth', category: 'SSH', title: 'Password auth disabled', description: '', status: 'warn', details: 'PasswordAuthentication = yes', severity: 'high' },
        { id: 'ssh-port', category: 'SSH', title: 'SSH port changed', description: '', status: 'info', details: 'SSH Port = 22', severity: 'low' },
        { id: 'ssh-max-auth', category: 'SSH', title: 'Max auth tries limited', description: '', status: 'pass', details: 'MaxAuthTries = 3', severity: 'medium' },
        { id: 'fail2ban-running', category: 'Intrusion Prevention', title: 'Fail2Ban running', description: '', status: 'pass', details: 'Fail2Ban active with 2 jails', severity: 'high' },
        { id: 'no-extra-uid0', category: 'Users', title: 'No extra UID 0', description: '', status: 'pass', details: 'Only root has UID 0', severity: 'critical' },
        { id: 'login-users', category: 'Users', title: 'Login users', description: '', status: 'info', details: '3 users can log in: root, deploy, admin', severity: 'info' },
        { id: 'security-updates', category: 'Updates', title: 'Security updates', description: '', status: 'warn', details: '3 security updates pending', severity: 'medium' },
        { id: 'all-updates', category: 'Updates', title: 'Packages up to date', description: '', status: 'info', details: '12 package updates available', severity: 'low' },
    ];
    ...
    return {
        ...
        checks,
        firewall: { available: true, backend: 'ufw', enabled: true, defaultIncoming: 'deny', defaultOutgoing: 'allow', rulesCount: 8 },
        fail2ban: {
            available: true, running: true, totalBanned: 23,
            jails: [
                { name: 'sshd', enabled: true, currentlyBanned: 12, totalBanned: 156, bannedIps: ['192.168.1.100', '10.0.0.50'] },
                { name: 'nginx-http-auth', enabled: true, currentlyBanned: 11, totalBanned: 89, bannedIps: ['172.16.0.15'] },
            ],
        },
        ssh: {
            permitRootLogin: 'no',
        ...
        recentLogins: [
            { user: 'admin', ip: '192.168.1.10', timestamp: 'Mon Mar 10 14:23', success: true, method: 'ssh' },
            { user: 'deploy', ip: '10.0.0.5', timestamp: 'Mon Mar 10 12:01', success: true, method: 'ssh' },
            { user: 'root', ip: '203.0.113.42', timestamp: 'Sun Mar  9 23:45', success: false, method: 'ssh' },
        ],
        users: [
            { username: 'root', uid: 0, gid: 0, home: '/root', shell: '/bin/bash', hasPassword: true, isSystem: false, canLogin: true },
            { username: 'deploy', uid: 1000, gid: 1000, home: '/home/deploy', shell: '/bin/bash', hasPassword: true, isSystem: false, canLogin: true },
            { username: 'admin', uid: 1001, gid: 1001, home: '/home/admin', shell: '/bin/bash', hasPassword: true, isSystem: false, canLogin: true },
        ],
        pendingUpdates: [
            { package: 'openssl', currentVersion: '3.0.2-0ubuntu1.14', newVersion: '3.0.2-0ubuntu1.15', isSecurity: true },
            { package: 'libssl3', currentVersion: '3.0.2-0ubuntu1.14', newVersion: '3.0.2-0ubuntu1.15', isSecurity: true },
            { package: 'curl', currentVersion: '7.81.0-1ubuntu1.15', newVersion: '7.81.0-1ubuntu1.16', isSecurity: true },
        ],
        summary: {
            totalChecks: 10,
            passed: 5,
...
async function getSnapshot(): Promise<SecuritySnapshot> {
    ...
    try {
        ...
        const criticalIssues = checks.filter(c => c.status === 'fail' && (c.severity === 'critical' || c.severity === 'high')).length;
