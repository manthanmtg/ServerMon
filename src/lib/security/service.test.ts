    ...
    describe('getSnapshot', () => {
        it('should return live security snapshot with correctly parsed status', async () => {
            mockExec({
                ...
                'fail2ban-client status sshd': 'Currently banned: 2\nTotal banned: 10\nBanned IP list: 1.1.1.1 2.2.2.2\n',
                'fail2ban-client status nginx': 'Currently banned: 1\nTotal banned: 5\nBanned IP list: 3.3.3.3\n',
        ...
        it('should fallback to iptables if ufw is not installed', async () => {
            (execFile as unknown as { mockImplementation: (fn: (...a: unknown[]) => void) => void }).mockImplementation(
                (...args: unknown[]) => {
                    const cmd = args[0] as string;
                    const callback = args[args.length - 1] as (err: Error | null, result: { stdout: string; stderr: string }) => void;
                    if (cmd === 'ufw') callback(new Error('command not found'), { stdout: '', stderr: 'command not found' });
                    else if (cmd === 'iptables') callback(null, { stdout: 'Chain INPUT (policy ACCEPT)\ntarget prot opt source destination\n', stderr: '' });
