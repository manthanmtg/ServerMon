...
class SystemUpdateService {
    ...
    private activeRuns = new Map<string, UpdateRunStatus & { logFile: string; metadataFile: string }>();
    ...
    public async listUpdateRuns(): Promise<UpdateRunStatus[]> {
        ...
        try {
            ...
            const runs = await Promise.all(
                metadataFiles.map(async f => {
                    try {
                        ...
                        if (metadata.status === 'running' && metadata.pid > 0 && !this.isProcessRunning(metadata.pid)) {
            ...
            runs.forEach(r => { if (r) runMap.set(r.runId, r); });
