    ...
    describe('getSnapshot', () => {
        it('should parse apt, npm, and pip updates correctly', async () => {
            ...
            mockExec({
                ...
                'npm outdated -g --json': JSON.stringify({ 'typescript': { current: '5.0.0', latest: '5.4.0' } }),
                'pip list --outdated --format=json': JSON.stringify([{ name: 'requests', version: '2.31.0', latest_version: '2.32.0' }]),
        ...
        it('should detect pending restart on Debian/Ubuntu', async () => {
            vi.mocked(si.osInfo).mockResolvedValue({ distro: 'Ubuntu', release: '24.04' } as unknown as si.Systeminformation.OsData);
