...
class NetworkService {
    ...
    private buildMockSnapshot(): NetworkSnapshot {
        ...
        return {
            ...
            connections: [
                { protocol: 'tcp', state: 'ESTABLISHED', localAddress: '127.0.0.1', localPort: '8912', peerAddress: '127.0.0.1', peerPort: '45321', process: 'node' },
                { protocol: 'tcp', state: 'LISTEN', localAddress: '0.0.0.0', localPort: '22', peerAddress: '0.0.0.0', peerPort: '*', process: 'sshd' }
            ],
            alerts: [],
    ...
    public async getSnapshot(): Promise<NetworkSnapshot> {
        ...
        if (this.shouldUseMockMode()) {
            ...
            try {
                ...
                const ifaceList: NetworkInterface[] = (Array.isArray(interfaces) ? interfaces : [interfaces]).map(i => ({
                    iface: i.iface,
                    ip4: i.ip4,
                    ip6: i.ip6,
                    mac: i.mac,
                ...
                const statList: NetworkStats[] = (Array.isArray(stats) ? stats : [stats]).map(s => {
                    const statsData = s as unknown as { rx_packets: number; tx_packets: number; [key: string]: unknown };
                    return {
                        iface: s.iface,
                        rx_bytes: s.rx_bytes,
                        tx_bytes: s.tx_bytes,
