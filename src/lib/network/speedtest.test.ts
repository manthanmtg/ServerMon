...
describe('SpeedtestService', () => {
  ...
  describe('runTest() — mock mode', () => {
    ...
    it('includes final result in last progress event', async () => {
      ...
      const resultPromise = service.runTest((p) => {
        finalProgress = p as { type: string; speed?: number; ping_ms?: number };
      });
      await vi.runAllTimersAsync();
      await resultPromise;
  ...
  describe('runTest() — concurrent protection', () => {
    ...
    it('emits error progress when already running', async () => {
      ...
      await service
        .runTest((p) => {
          errorProgress = p as { type: string; error?: string };
        })
        .catch(() => {});
      expect((errorProgress as { type: string; error?: string } | null)?.type).toBe('error');
      expect((errorProgress as { type: string; error?: string } | null)?.error).toContain(
        'already in progress'
      );
    });
  });
  ...
  describe('runTest() — system Ookla CLI', () => {
    it('resolves with parsed CLI result on success', async () => {
      ...
      if (stdoutCallback)
        (stdoutCallback as (data: Buffer) => void)(Buffer.from(resultJson + '\n'));
